...
async function preCheck() {
	return new Promise((resolve, reject) => {
		...
		shopeeHeaders.Cookie = $request.headers['Cookie'] || $request.headers['cookie'];
		config = {
		...
		for (let i = 1; i < UrlData.length; i++) {
			...
			*/
			...
			if (i == 5) {
				...
				// 如果 需要出門寵物 0 ，則不讓寵物休息
				if (config.petIDOut == -1 || config.petIDOut >= config.petIDOutS.length || config.petIDOutS.length == 0 || config.petIDInS.length == 0) { i++; await delay(1.0); }
				else {
