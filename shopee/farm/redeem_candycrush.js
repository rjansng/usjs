	...
	return new Promise((resolve, reject) => {
		...
		for (let i = item_list.length - 1; i >= 0; i--) {
			if (item_list[i].item_type == 4) // 4  蝦幣
			{
				...
				let re = new RegExp(`^${消消樂點數兌換蝦幣} 蝦幣`, 'i');
...
let UrlData = [[],
['GET', '取得兌換商店清單', '取得', 'https://games.shopee.tw/gameplatform/api/v2/redeem_store/item_list/store/115?guest=1&limit=100&offset=0&appid=AxJMo8pm7cs5ca7OM8&activity=1731357eb13431cb', '', , ProcData1],
['POST', '兌換 蝦幣', '兌換', 'https://games.shopee.tw/gameplatform/api/v2/redeem_store/redeem_item/store/115/item/{event_item}?appid=AxJMo8pm7cs5ca7OM8&activity=1731357eb13431cb', '', ['event_item'], ProcData2],
];
...
function preInit() {
	...
	消消樂點數兌換蝦幣 = $persistentStore.read('消消樂點數兌換蝦幣') || '';
	if (消消樂點數兌換蝦幣.match(/^[\d\.]+$/)) { 消消樂點數兌換蝦幣 = parseFloat(消消樂點數兌換蝦幣); } else { 消消樂點數兌換蝦幣 = 0; }
}
...
(async () => {
	...
	try {
		...
		let runCount = 0;
		let item = -1;
		if (消消樂點數兌換蝦幣 == 0) { flag = false; console.log('您指定：不兌換。'); }
		for (let i = 1; i < UrlData.length; i++) {
			...
			item = -1;
			if (i === 2) {
				DataPostBodyList[i].request_id = `${config.shopeeInfo.token.SPC_U}_115_${config.event_item}_${getToken()}_` + getRnd(7)
			}
