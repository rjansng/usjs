let title = '取得 手動 收成作物 數據';
let version = 'v20240130';
...
try {
	...
	if (body) {
		...
		if (json.code === 0) {
			const cropName = json.data.crop.meta.name;
			const luckyDrawAwardName = json.data.reward.rewardItems[0].itemExtraData.luckyDrawAwardName;
			console.log(`獲得 ${cropName} 🌳\t${luckyDrawAwardName}`);
			...
			try {
				...
				s.c++;
				s.f = true;
				s.d.push(`🌱${cropName}\t🌳${luckyDrawAwardName} : ${(new Date()).format('3')}`);
				ts[tsid] = s;
				tasks[tsn] = ts;
