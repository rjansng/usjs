...
if ($request.method != 'OPTIONS') {
	...
	if (flag) {
		...
		try {
			if (json.hasOwnProperty('user') && json.user.hasOwnProperty('game_stats')) {
				if (json.user.game_stats.ingame_rank != 1) {
					// $done({ response: { status: 404, headers: {}, body: '{}' } });
					try {
				...
				// json.user.remaining_booster.magnet = 0;
				// json.user.remaining_booster.speed_boost = 0;
				// json.user.remaining_booster.zoom_out = 0;
				// json.user.game_stats.booster_used.forEach(x => { x.amount = 0; });
