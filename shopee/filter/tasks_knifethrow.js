...
// let $request = {
// 	body: '{"code": 0,"data": {"maxCorrectRate": 70,"costTimeMS": 44770,"correctRate": 90},"timestamp": 1678965259274,"msg": ""}'
// 	, url: 'https://games.shopee.tw/gameplatform/api/v2/goluckydraw/module/200214/event/e39086600a30ebe2/lucky?appid=KKyN58ob6K6Nln3whP'
// };
// let $done = function () { };
...
shopee_ids = [
	{ 'id': 'A', 'name': '分享得1次', 't': '', 'h': '目前沒有 (曾經有)' },
	{ 'id': 'B', 'name': '玩過1次 ', 't': '', 'h': '果園任務10水滴' },
	{ 'id': 'C', 'name': '開始丟 ', 't': 'T', 'h': '5次 得點數可兌換獎勵' },
	{ 'id': 'CE', 'name': '得分   ', 't': 'S', 'h': '' },
	{ 'id': 'X', 'name': '獎勵    ', 't': '', 'h': '' },
	{ 'id': 'Z', 'name': '兌換獎勵', 't': '', 'h': '點數兌換獎勵' },
];
let reset = false;
...
let caption = '';
let url_list = [
	[1, 'C', 'https://games.shopee.tw/knife/api/v1/begin/activity/'],
	[1, 'CE', 'https://games.shopee.tw/knife/api/v1/end/activity/'],
	[1, 'X', '^https:\\/\\/games\\.shopee\\.tw\\/gameplatform\\/api\\/v2\\/goluckydraw\\/module\\/[^\\/]+\\/event\\/[^\\/]+\\/lucky\\?appid=.+$', 1],
];
// try {
...
if (json.code === 0) {
	...
	if (url_list.some(u => {
		...
		let s = ts[p.id];
		s.c++;
		if (p.id === 'CE' || p.id === 'X') { sC = ts['C']; s.c = sC.c; }
		let msg = `今天第 ${s.c} 次`;
		let msg2 = '';
		caption = p.name;
		// let sf = s.f;
		...
		if (p.id === 'C') {
			...
			console.log(d);
			json.data.basic.game_rounds.forEach(x => { x.bonus = 120; }); // 每關過關獎勵點數
			json.data.basic.game_time = 60; // 時間 預設50 不能太大
			//json.data.basic.prop_value = 20; // 每支預設 10 分
			for (let i = 0; i < json.data.random_prop_order.length; i++) {
				//json.data.random_prop_order[i] = Math.floor(Math.random() * 2) + 1;
				json.data.random_prop_order[i] = -1;
				// -1 空  0 減漫  1 隨機?  2 x2
			}
			console.log(d);
			...
			s.s += d.score; msg2 = `得分 ${d.score}`;
			if (d.hasOwnProperty('settlement')) {
				console.log(`lucky_draw_token:${d.settlement.lucky_draw_token}`);
				$persistentStore.write(d.settlement.lucky_draw_token, 'KnifeThrow_LuckyDrawToken');
			}
		}
		else if (p.id === 'X') { msg2 = '\n🔆' + d.package_name; s.r += msg2; }
		msg += msg2;
