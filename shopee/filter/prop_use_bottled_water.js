...
async function ProcData2(data, dc) {
	return new Promise((resolve, reject) => {
		...
		if (json.code === 0) {
			...
			props.forEach(ps => {
				found2 = false;
				if (ps.itemId == config.propItemId) { found2 = true; found = found2; }
				console.log(`${ps.itemId}\t${[ps.name]}\t${[ps.parameter]} ${[ps.itemName]}\t${ps.amount}/${ps.stackLimit}\t${(found2 ? '找到' : '')}`);
			});
			if (!found) { console.log('\n您目前沒有 ' + config.propItemName); }
			console.log('');
		}
		else {
			return reject([`執行失敗 ‼️`, json.msg, data]);
...
let UrlData = [[],
['GET', '取得ℹ️作物資訊', '1', 'https://games.shopee.tw/farm/api/orchard/context/get?skipGuidance=0', '', , ProcData1],
['GET', '顯示道具清單', '2', 'https://games.shopee.tw/farm/api/prop/backpack/list?t={token}', '', ['token'], ProcData2],
['POST', '使用道具', '3', 'https://games.shopee.tw/farm/api/orchard/crop/use_bottled_water', '', , ProcData3],
];
...
function preInit() {
	let argd = 0;
	try {
		if ($argument && $argument.trim() != '') { argd = parseInt($argument.trim()); }
	} catch (e) { console.log(e); }

	config.propItemId = argd;
	config.propItemName = '';
	if (argd > 0) {
		item_list.some(x => { if (x.id == argd) { config.propItemName = x.name; return true; } });
	}
}
