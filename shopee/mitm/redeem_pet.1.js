...
let LimitPoints = parseInt($persistentStore.read('3蝦幣限點') || '1200');

const caption = '兌換 3 蝦幣';
				...
				if (item_list[i].name == '3 蝦幣') {
					...
					if (item_list[i].redeem_limit > item_list[i].redeemed_times) {
						if (item_list[i].left_amount > 0) {
							if (json.data.current_points < LimitPoints) {
								msg += `\n點數 < ${LimitPoints}，不兌換。`;
								m = 5;
							}
							else if (json.data.current_points > item_list[i].points_to_redeem) {
							...
							// try {
							// 	// let dts = item_list[i].display_start_time * 1000;
							// 	// let dtn = new Date(new Date().format("yyyy/MM/dd")).getTime();
							// 	// console.log('\t\t\tS:' + new Date(dts).format());
							// 	// console.log('\t\t\tN:' + new Date(dtn).format());
							// 	// msg += '\n' + new Date(dts).format();
							// 	//if (dts != dtn) {
							// 	if (!config.rerun1) { config.rerun1 = true; msg += '\n嘗試重載。'; }
							// 	//}
...
async function ProcData2(data, dc) {
	...
	return new Promise((resolve, reject) => {
		...
		else { msg = '失敗'; if (!config.rerun2) { msg += '重試'; config.rerun2 = true; } }
		try {
			let dataName = 'ShopeeGamePlayed' + _ShopeeUserID;
			let tsn = 'pets';
			let tsid = 'Z';
